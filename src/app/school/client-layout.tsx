...
export default function ClientLayout({ children, school }: { children: React.ReactNode, school: { name: string, city: string } | null }) {
  ...
  const hideLayout = pathname.startsWith("/school/book") || pathname.startsWith("/school/auth");
  ...
  const handleLogout = async () => {
    // Calling our server action to clear cookie
    const { logoutSchool } = await import("@/app/actions/schoolAuth");
    await logoutSchool();
  };
      ...
      <div className="p-4 space-y-2" style={{ borderTop: "1px solid var(--school-border)" }}>
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-slate-400 truncate">{school.city}</p>
          </div>
          <Building2 className="w-4 h-4 text-slate-300 shrink-0" />
        </div>
        ) : (
          <Link href="/school/auth" className="flex items-center gap-3 p-3 rounded-xl cursor-pointer transition-colors hover:bg-slate-50 border border-slate-200">
             <div className="flex-1 min-w-0 text-center"><p className="text-sm font-bold text-slate-900 truncate">Login Required</p></div>
          </Link>
        )}
        ...
        {school && (
        <button onClick={handleLogout} className="w-full flex items-center gap-2 px-3 py-2 text-sm text-slate-400 hover:text-red-500 font-semibold transition-colors rounded-xl hover:bg-red-50">
