...
export default function SchoolAuth() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex">
        ...
        <div className="relative z-10 space-y-8">
          <div>
            ...
            <h1 className="text-4xl font-extrabold text-slate-900 leading-tight mb-4 tracking-tight">
              Bring India's greatest
              <br />
              <span className="text-indigo-600">to your classrooms.</span>
            </h1>
            <p className="text-slate-500 text-lg leading-relaxed max-w-md">
              Securely book Olympians and national champions for mentoring, speeches, and training sessions backed by our 100% Escrow safe-guard.
            </p>
          </div>
      ...
      {/* Right Auth Panel */}
      <div className="w-full lg:w-1/2 flex flex-col items-center justify-center p-6 md:p-12 relative bg-slate-50">
        <div className="w-full max-w-md">
          <BackButton />
          <Link href="/" className="lg:hidden flex items-center gap-2 mb-10 mt-6">
          <div className="w-9 h-9 rounded-xl bg-gradient-to-tr from-indigo-600 to-indigo-400 flex items-center justify-center font-black text-white">
            DA
          </div>
        ...
        <div className="w-full max-w-md">
          ...
          {mode === "signup" ? <SignupCard router={router} setMode={setMode} /> : <LoginCard router={router} />}
        </div>
      </div>
    </div>
    </div>
...
function SignupCard({ router, setMode }: { router: any, setMode: any }) {
  ...
  return (
    <form onSubmit={handleSignup} className="space-y-6">
      ...
      <div className="space-y-4">
        <div className="space-y-1.5">
          ...
          <input
            ...
            className="w-full border border-slate-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all font-medium"
          />
        </div>
        <div className="space-y-1.5">
        ...
        <div className="space-y-1.5">
          ...
          <div className="flex gap-2">
            ...
            <input
              ...
              className="flex-1 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all font-medium"
            />
          </div>
          <p className="text-xs text-slate-400 mt-1">This will be your login ID.</p>
        </div>
      </div>
      ...
      <button
        type="submit"
        disabled={loading}
        className="w-full bg-indigo-600 text-white py-4 rounded-xl font-bold flex items-center justify-center gap-2 hover:bg-indigo-700 transition-all shadow-sm hover:shadow-indigo-500/25 disabled:opacity-50"
      >
        {loading ? "Creating Account..." : "Register Institution"} <ChevronRight className="w-4 h-4" />
      </button>

      <p className="text-center text-xs text-slate-400">
        By registering, you agree to DeshKa Athlete&apos;s Policy terms.
      </p>
    </form>
  );
}
    ...
    <form onSubmit={handleLogin} className="space-y-6">
      <div>
        ...
        <p className="text-slate-500 text-sm">
          Log in to your school portal to manage pending books and escrows.
        </p>
      </div>
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-1.5">
          <label className="block text-sm font-bold text-slate-700">Login OTP</label>
          <input
            type="password"
            placeholder="Mock OTP (optional)"
            className="w-full border border-slate-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all font-medium"
          />
        </div>
      </div>
      ...
      <button
        type="submit"
        disabled={loading}
        className="w-full bg-slate-900 text-white py-4 rounded-xl font-bold flex items-center justify-center gap-2 hover:bg-slate-800 transition-all shadow-sm hover:shadow-slate-500/25 disabled:opacity-50"
      >
